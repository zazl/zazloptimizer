...
function compress(content, path) {
	var result = UglifyJS.minify(content);
	if (result.error) {
		console.log("Error minifying ["+path+"] "+result.error.message+" at "+result.error.line+":"+result.error.pos);
		return content;
	} else {
		return result.code;
	}
				...
				resourceloader.getTimestamp(bootstrapModulePath, function(ts) {
					...
					if (cacheEntry && cacheEntry.ts === ts) {
						...
						resourceloader.readText(bootstrapModulePath, function(text) {
							var compressed = compress(text, bootstrapModulePath);
        ...
        resourceloader.getTimestamp(dependencyPath, function(ts) {
			...
			if (keepgoing === true) {
				resourceloader.readText(dependencyPath, function(content) {
					...
					if (compressContent) {
						var compressed = compress(content, dependencyPath);
						responseWriter.write(compressed);
	...
	resourceloader.readText("zazlConfig.json", function(configText) {
		...
		if (configpath) {
			fs.exists(configpath, function (exists) {
				var userConfigPath = fs.realpathSync(configpath);
				config.amdconfig.userConfigPath = path.dirname(userConfigPath);
				if (exists) {
					fs.readFile(configpath, 'utf8', function(err, configText) {
						if (err) {
							console.log("Failed to read config file ["+configpath+"] err : "+err+" Using default config");
							cb(config);
						} else {
							var userconfig = JSON.parse(configText);
							cb(util.mixin(config, userconfig));
						}
					});
				} else {
					cb(config);
...
var Optimizer = function(params) {
	if (util.isArray(params.appdir)) {
		params.appdir.forEach(function(appdir) {
	...
	loadConfig(params.configpath, function(config) {
		this.config = config;
		if (params.readycb) {
			params.readycb(this);
		}
...
var ConnectOptimizer = function(params) {
	if (util.isArray(params.appdir)) {
		params.appdir.forEach(function(appdir) {
			resourceloader.addProvider(appdir);
		});
...
exports.createConnectOptimizer = function(appdir, compress, readycb) {
	var params;
	if (util.isObject(appdir)) {
		params = appdir;
	} else {
	...
	return new ConnectOptimizer(params);
};
